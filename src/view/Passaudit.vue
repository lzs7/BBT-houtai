    ...
    <el-dialog title="详情" :visible.sync="centerDialogVisible" width="50%" center>
      <el-form :model="tableData1">
        ...
        <el-form-item label="实名状态" :label-width="formLabelWidth">
          <el-input v-model="tableData1.userState==3?'实名通过':''" autocomplete="off" :disabled="true"></el-input>
        </el-form-item>
        <el-form-item label="确认信息" :label-width="formLabelWidth">
          <el-input v-model="tableData1.testState==1?'已确认':''" autocomplete="off" :disabled="true"></el-input>
        </el-form-item>
      </el-form>
    </el-dialog>
  </div>
...
<script>
import { getnewuser, getuser, select } from '../api/api'
...
export default {
  ...
  mounted () {
    let cookie = this.common.getCookie() // 获取cooki
